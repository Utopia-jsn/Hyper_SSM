...
def get_params():
    params = {
        'weight': './runs/train/exp9/weights/best.pt', # 这选择想要热力可视化的模型权重路径
        'cfg': './ultralytics/cfg/models/mamba-yolo/hyper-mamba24-B.yaml', # 这里选择与训练上面模型权重相对应的.yaml文件路径
        'device': 'cuda:0', # 选择设备，其中0表示0号显卡。如果使用CPU可视化 # 'device': 'cpu'
        'method': 'GradCAMPlusPlus', # GradCAMPlusPlus, GradCAM, XGradCAM
        'layer': 'model.model[5]',   # 选择特征层
        'backward_type': 'all', # class, box, all
        'conf_threshold': 0.6, # 置信度阈值默认0.65， 可根据情况调节
...
if __name__ == '__main__':
    ...
    model('dark_20200720002223.jpg', './result') # 第一个参数是图片的路径，第二个参数是保存路径，比如是result的话，其会创建一个名字为result的文件夹，如果result文件夹不为空，其会先清空文件夹。
