...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, XSSBlock, MANet, C3k2, C2PSA, A2C2f}:
        ...
        #####attention  ####
        elif m in {CBAM, SpatialAttention}:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
...
def guess_model_task(model):
    ...
    #     print(f"cfg['head']: {cfg['head']}")  # Debug print statement
    ...
    #     if m == "obb":
    #         return "obb"
    #     # If none of the above conditions are met, return a default or raise an error
    #     return "default"  # Or raise ValueError("Unknown head module type.")
    ...
    def cfg2task(cfg):
        ...
        if m == "obb":
            ...
            return 'detect'
        raise ValueError(f"Unknown head module type: {m}")
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect)) or isinstance(m, ASFF_Detect):
                return "detect"
